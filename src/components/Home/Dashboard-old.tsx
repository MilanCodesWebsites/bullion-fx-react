...
import { TrendingUp, TrendingDown, DollarSign, Clock, CheckCircle, XCircle, AlertCircle, Quote, Target, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2 sm:gap-3">
          <a
            href="/deposit"
            className="flex items-center gap-2 px-4 py-2.5 bg-vertex-blue-600 hover:bg-vertex-blue-700 text-white font-medium rounded-lg transition-colors duration-200"
          >
          ...
          <a
            href="/withdraw"
            className="flex items-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-medium rounded-lg border border-slate-700 transition-colors duration-200"
          >
            <DollarSign className="w-4 h-4" />
            Withdraw
          </a>
      ...
      {/* Wallet Balance - modern glass card with sparkline */}
      <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-br from-[#0b1220] via-[#0b1322] to-[#0b1220] p-4 sm:p-6 lg:p-8 shadow-[0_0_40px_-10px_rgba(59,130,246,0.35)]">
        ...
        <div className="relative flex items-center justify-between mb-4 sm:mb-6">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-xl bg-vertex-blue-600/15 border border-vertex-blue-600/30 flex items-center justify-center">
              <DollarSign className="w-4 h-4 sm:w-5 sm:h-5 text-vertex-blue" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-semibold text-white">Wallet Balance</h2>
              <p className="text-xs text-slate-400">Live portfolio overview</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowBalance(s => !s)}
              className="inline-flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-xs sm:text-sm text-slate-300 hover:bg-slate-700/60 transition"
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 items-center">
          <div className="lg:col-span-2">
            <div className="flex items-baseline gap-3">
              <div className={`text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white ${!showBalance ? 'blur-sm select-none' : ''}`}>
                ${totalBalance.toFixed(2)}
              </div>
              <span className="text-xs sm:text-sm px-2 py-1 rounded-md bg-slate-800/70 border border-slate-700 text-slate-300">USD</span>
            </div>
            <div className="mt-3 text-slate-400 text-xs sm:text-sm">Updated just now</div>
        ...
        <div className="relative mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 sm:p-4">
            <p className="text-xs sm:text-sm text-slate-400">Initial Balance</p>
            <p className="mt-1 text-base sm:text-lg font-semibold text-white">${initialBalance.toFixed(2)}</p>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 sm:p-4">
            <p className="text-xs sm:text-sm text-slate-400">Profit & Loss</p>
            <div className="mt-1 flex items-center gap-2">
              {pnlAmount >= 0 ? (
                <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-vertex-blue" />
              ) : (
                <TrendingDown className="w-4 h-4 sm:w-5 sm:h-5 text-red-400" />
              )}
              <p className={`text-base sm:text-lg font-semibold ${pnlAmount >= 0 ? 'text-green-400' : 'text-red-400'}`}>${Math.abs(pnlAmount).toFixed(2)}</p>
            </div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 sm:p-4">
            <p className="text-xs sm:text-sm text-slate-400">P&L Percentage</p>
            <p className={`mt-1 text-base sm:text-lg font-semibold ${pnlPercentage >= 0 ? 'text-green-400' : 'text-red-400'}`}>{pnlPercentage.toFixed(2)}%</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6">
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between p-3 sm:p-4 bg-slate-800 rounded-xl">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="p-2 bg-vertex-blue-600/15 rounded-lg">
                    <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-vertex-blue" />
                  </div>
                  <div>
          ...
          <div className="bg-gradient-to-br from-[#0b1220] via-[#0c1426] to-[#0b1220] border border-slate-800 rounded-2xl p-4 sm:p-6">
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
              <div className="p-2 bg-vertex-blue-600/15 rounded-lg">
                <Quote className="w-4 h-4 sm:w-5 sm:h-5 text-vertex-blue" />
              </div>
              <div>
