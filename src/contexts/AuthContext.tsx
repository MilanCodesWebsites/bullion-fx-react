...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkExistingSession = async () => {
      try {
        ...
        const adminAuthenticated = sessionStorage.getItem('bullionfx_admin_authenticated') ?? sessionStorage.getItem('optima_admin_authenticated');
  ...
  const adminLogin = async (email: string, password: string): Promise<boolean> => {
    ...
    const adminCredentials = [
      // Back-compat Optima and new BullionFX admin accounts
      { email: 'Ogolomaprince1@gmail.com', password: 'Mynewaccount@2004' },
      { email: 'admin@bullionfx.com', password: 'BullionFX@Secure2025' },
      { email: 'bullionfx2@proton.me', password: 'TUMMYtummy123#' },
    ...
    if (isValidAdmin) {
      setIsAdminAuthenticated(true);
  // Store admin authentication in session storage (new keys)
